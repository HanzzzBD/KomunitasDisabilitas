...
Preset bersama (shared config) untuk seluruh workspace Nawasena: TypeScript, ESLint, Prettier, dan **ESLint boundaries** (arsitektur sebagai kode).
...
| `@nawasena/config/prettier`          | konfigurasi Prettier bersama                       |
| `@nawasena/config/eslint`            | preset ESLint dasar (semua app & paket)            |
| `@nawasena/config/eslint/boundaries` | preset ESLint + **boundaries** (khusus `apps/api`) |
...
```js
// .eslintrc.cjs
module.exports = require("@nawasena/config/eslint");
```
...
```bash
pnpm --filter @nawasena/config test
```
