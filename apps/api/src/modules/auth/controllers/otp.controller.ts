...
export function createOtpController(service: OtpService) {
  return {
    ...
    /** POST /auth/otp/verify → 200 dengan userId (JWT menyusul di PR-018). */
    async verify(req: Request, res: Response): Promise<void> {
      const data = await service.verify(req.body as VerifyOtp, actorOf(req));
