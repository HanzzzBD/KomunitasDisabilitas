...
* [x] Login Google end-to-end (akun uji). — **PR-030c membangunnya; verifikasi manual 2026-08-10 menutupnya.** Alur PKCE diuji terhadap vektor resmi RFC 7636, `state` anti-login-CSRF diperiksa, titipan sekali pakai. **Dijalankan terhadap Google sungguhan** dengan kredensial OAuth nyata dan akun uji nyata: URL persetujuan diperiksa dari address bar (`client_id`, `redirect_uri=http://localhost:5173/masuk/google`, `code_challenge_method=S256`, `state`), penukaran code berhasil, dan sesi terbit — `AUTH_LOGIN_SUCCEEDED {"method":"google"}` di `audit_logs` plus refresh token baru berstatus aktif. Cookie refresh terbukti DIPAKAI peramban, bukan sekadar diterima: halaman kembalian dimuat dari nol, dan `DELETE /auth/account` yang menyusul menuntut access token yang satu-satunya sumbernya `pulihkanSesi()` → `/auth/refresh` → cookie HttpOnly. Rinciannya di [log](log/implementation_log_phase03.md#verifikasi-manual-google-oauth--tahap-0-authtime).
...
* [x] Accessibility Test (axe + NVDA dialog) — **axe lolos di dua lapis** (jsdom + Chromium): dialog hapus akun pada langkah yang benar-benar memuat tombol perusaknya (PR-033c-1) dan layar kembalian Google (PR-033c-2). **NVDA dijalankan 2026-08-11** atas jalur Google, empat butir lulus (lihat AC nomor 3). Jalur OTP belum — tidak ada provider OTP di lingkungan uji.
* [x] Manual Verification (akun uji dihapus benar-benar) — **selesai 2026-08-10, jalur Google.** Akun uji nyata dihapus terhadap API dev, dan hasilnya diperiksa di basis data — bukan disimpulkan dari layar: `deleted_at` terisi (soft delete, konsisten dengan janji 30 hari), `token_version` naik, **0 refresh token aktif / 10 dicabut**, dan `audit_logs` mencatat `ACCOUNT_DELETED` `{"stage":"requested"→"completed","method":"google","revokedCount":6}`. `method: "google"` itu yang menentukan: server memverifikasi consent Google terhadap `google_id` akun, jadi jalur re-auth benar-benar ditempuh dan tidak diam-diam jatuh ke jalur lain. **Jalur OTP belum** — lingkungan ini tidak punya provider OTP, sama seperti AC 1 PR-030.
...
* [x] Dialog konfirmasi lolos NVDA checklist. — **Dijalankan dengan NVDA sungguhan 2026-08-11** (NVDA + Chrome, Speech Viewer sebagai bukti tertulis), setelah butir strukturalnya lebih dulu ditegakkan CI di PR-033c-1. Empat butir yang tidak bisa dijamin mesin, semuanya lulus: (1) keempat akibat — termasuk kalimat 30 hari — terbaca **sebelum** tombol "Saya mengerti, lanjutkan"; (2) pergantian isi dialog **di tempat** dari langkah 1 ke langkah Google diumumkan lengkap dengan judul dan deskripsi barunya; (3) layar kembalian sesudah muat ulang penuh membacakan `h1` + akibatnya, dan **jalan keluar diumumkan lebih dulu** daripada tombol perusak; (4) layar "Akun Anda sudah dihapus" — pergantian di tempat lagi — ikut diumumkan berikut kalimat 30 hari. Dua butir tengah adalah yang paling mungkin gagal (pergantian isi di dalam dialog yang sudah terbuka kerap senyap) dan keduanya bekerja. **Cakupan yang TIDAK diuji dicatat jujur:** jalur OTP langkah 2 (tidak ada provider OTP di lingkungan uji), penanda tunggu pemulihan sesi (selesai dalam milidetik di localhost — butuh jaringan ter-throttle), dan peramban selain Chrome. Transkrip lengkap di [log](log/implementation_log_phase03.md#verifikasi-nvda--dialog-hapus-akun--layar-kembalian-google).
* [x] Seluruh halaman keyboard-only. *(Tiga dari empat alur tertutup sebagai PERBUATAN keyboard, bukan lewat pemeriksaan `href`: navigasi panel (PR-033a — Tab lalu Enter), kendali ekspor (PR-033b — di jsdom DAN di peramban), dan hapus akun jalur OTP dari membuka dialog sampai penghapusan terkirim (PR-033c-1). Seluruh tombol pada alur ini memakai `aria-disabled`, bukan `disabled`: tombol yang dinonaktifkan saat sedang MEMEGANG fokus melemparkan fokus itu ke awal dokumen, dan di dalam dialog itu berarti keluar dari jerat fokusnya. **Jalur keempat — hapus akun lewat Google — ditutup 2026-08-14 di peramban sungguhan, tanpa satu pun klik**: 5 Tab ke "Hapus akun saya" → Enter membuka dialog (fokus mendarat di dalamnya) → 2 Tab + Enter ke "Saya mengerti, lanjutkan" → Enter pada "Lanjut ke Google" → 2 Tab + Enter memilih akun di halaman Google → 3 Tab ke "Hapus akun saya sekarang" → Enter, dan akunnya benar-benar terhapus (`deleted_at` terisi, `token_version` naik, 0 sesi aktif / 9 dicabut, `ACCOUNT_DELETED` `requested`→`completed`). Setiap elemen yang dilewati berpenanda fokus `solid 2.4px`, dan pada kedua layar berbahaya jalan keluar selalu mendahului tombol perusak. **AC ini sempat diblokir cacat nyata, bukan kekurangan test:** tiga percobaan pertama tersandung layar "sesi habis" yang menyembunyikan tombol final — akar masalahnya balapan refresh yang diperbaiki PR-033i.)*
...
* **KOREKSI 2026-08-10 atas butir di atas — `auth_time` ternyata TIDAK PERNAH ADA.** Kalimat "penegakannya terbaca dari klaim `auth_time`" ditulis sebagai rencana, dan pengukuran membantahnya: **0 dari 16 penukaran id_token** memuat klaim itu, termasuk **4 penukaran yang membawa `max_age=0`**. Google menerima dan mem-parsing `max_age` (`opparams=?max_age=0`), perilakunya bergantung pada state sesi (email diketik manual → password diminta; akun bersesi dipilih → lewat begitu saja), tetapi **hasilnya tidak tersedia sebagai klaim yang bisa diverifikasi server**. Enforcement berbasis `auth_time` karena itu **DIBATALKAN** (keputusan owner), bukan ditunda — gerbang atas klaim yang tidak pernah tiba hanya menghasilkan cabang mati yang terbaca seperti perlindungan. **Klaim keamanan diturunkan:** jalur Google membuktikan peramban memegang sesi Google yang cocok dengan `google_id`, **bukan** kepemilikan akun saat ini; ia lebih lemah daripada kode OTP baru. Lapisan pemulihannya dipindahkan ke pemberitahuan pasca-hapus — dicatat sebagai dependensi eksplisit di [Phase 07 PR-049](phase-07-notifications.md).
...
* 2026-08-10 — **Verifikasi manual Google OAuth dijalankan** (owner + agent, peramban sungguhan terhadap Google sungguhan). Menutup utang manual PR-033c-2, membatalkan rencana enforcement `auth_time`, dan menurunkan klaim keamanan jalur Google. Lihat [log/implementation_log_phase03.md](log/implementation_log_phase03.md#verifikasi-manual-google-oauth--tahap-0-authtime).
